...
public class LogOutput : MonoBehaviour
{
    ...
    void OnApplicationQuit()
    {
        ...
        for (int i = 0; i < transform.childCount; i++) // Writes the data for each player to the file
        {
            Log(transform.name, "PlayerDataLog", 
                (i + 1).ToString(),

                transform.GetChild(i).GetComponent<StickToPlanet>().l_hangTime.ToString(), 
                transform.GetChild(i).GetComponent<StickToPlanet>().l_groundTime.ToString(),
                transform.GetChild(i).GetComponent<Shoot>().l_bullets.ToString(),
                transform.GetChild(i).GetComponent<PlayerBoost>().l_boosts.ToString(),
                transform.GetChild(i).GetComponent<KnockOut>().l_deaths.ToString()
                );
        }
    }
